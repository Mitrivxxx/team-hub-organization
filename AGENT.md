## Purpose
- Team microservice skeleton (Web API) for Team Hub.
...
- Prod Env (Docker): Host port `5002` -> container `8080`. Container `team-hub-team-prod`.
...
- Shared deps: dual `actions/checkout` — monorepo `Mitrivxxx/team-hub@main` then overlay this repo at `services/team-hub-team`.
- Image: `ghcr.io/<owner>/team-hub-team` (`latest` + short commit SHA on `main` push). Uses `GITHUB_TOKEN` (no extra secrets).
